...
export class APIStack extends cdk.Stack {
    constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
        ...
        // go のコードの場所
        const entry = path.join(__dirname, '../functions/discordbot');
        // go build 用の環境変数
        const buildEnv = {
            CGO_ENABLED: '0',
            GOOS: 'linux',
        ...
        // kms ssm key
        const kmskey = kms.Key.fromLookup(this, `KMSKey`, { aliasName: 'alias/aws/ssm' })
            ...
            new iam.Policy(this, `Policy`, {
                statements: [
                    new iam.PolicyStatement({
                        ...
                        resources: [`arn:aws:ssm:${this.region}:${this.account}:parameter${ssmpath}/*`],
                    })
                ],
            }),
        )
